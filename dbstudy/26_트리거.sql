/*
    ...
    3. DML 직전에 수행되는  BEFORE 트리거, 직후에 수행퇴는 AFTER 트리거기 있음
...
CREATE OR REPLACE TRIGGER TRIG1
    AFTER -- AFTER OR BEFORE 만 들어갈 수 있음
    UPDATE -- INSERT OR UPDATE OR DELETE 만 들어갈 수있음
...
-- 1) 퇴사자 테이블 만들기(EMPLOYEES 테이블과 동일한 구조, 데이터 없이 복사)
    DROP TABLE RETIRES;
   CREATE TABLE RETIRES AS (SELECT * FROM EMPLOYEES WHERE 1 = 2 );
...
END RETIRE_TRIG;
-- 6) EMPLOYEE 테이블의 데이터 DELETE 수행하기 (RETIRE_TRIG 동작확인)
