...
BEGIN
    SELECT FIRST_NAME -- FIRST_NAME 칼럼값을
...
CREATE OR REPLACE PROCEDURE PROC3(EMP_ID IN EMPLOYEES.EMPLOYEE_ID%TYPE) --%TYPE =~% 타입과 같은 타입
IS
    FNAME EMPLOYEES.FIRST_NAME%TYPE;
BEGIN
    SELECT FIRST_NAME
...
END PROC3;
-- 프로시저 PROC3 호출
EXECUTE PROC3(100); -- 100 => EMPLOYEE_ID =>EMP_ID
EXECUTE PROC3(101);
EXECUTE PROC3(500); -- 없는번호 출력 안됨 NODATAFOUND 예외처리해놓으면 깔끔
...
CREATE OR REPLACE PROCEDURE PROC4(FNAME OUT EMPLOYEES.FIRST_NAME%TYPE) -- FNAME이 파라미터방식으로 저장됨
IS -- 적을 거 없으면 생략 가능
BEGIN
    SELECT FIRST_NAME
      INTO FNAME
...
-- 프로시저 PROC4 호출 프로시저4가 여기로 값을 저장
DECLARE -- PLSQL
    FNAME EMPLOYEES.FIRST_NAME%TYPE; -- 출력 파라미터로 사용할 변수
BEGIN
    --EXECUTE PROC4(FNAME); --  PLSQL 내부에서 프로시저를 호출할 땐 EXECUTE 생략
    PROC4(FNAME);
    DBMS_OUTPUT.PUT_LINE(FNAME);
END;
...
BEGIN
    ...
    VALUES(BUY_SEQ.NEXTVAL, C_NO, P_CODE, BUY_AMT); -- BUYNO시퀀스에서 번호빼서 사용
    -- 2) 고객 테이블의 고객포인트를 UPDATE 한다. (총 구매액의 10% 적립)
    UPDATE CUSTOMER
        SET CUST_POINT = CEIL((SELECT PROD_PRICE
                            FROM PRODUCT
