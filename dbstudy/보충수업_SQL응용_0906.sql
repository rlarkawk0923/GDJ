...
-- 프로시저 실행은 학과 번호 = 101 학과를 대상으로 실행
-- 프로시저 이름은 my_proc
...
SELECT DISTINCT GET_STUDENT('Dals')--결과값 중복제거
FROM STUDENT;

SELECT GET_STUDENT(USERID) -- 결과값 1개
FROM STUDENT
WHERE USERID = 'Dals';

-- 트리거
-- 샘플SAMPLE 테이블의 행을 갱신, 삭제 이후 'SampleTrg 동작' 서버 메시지 출력하기
