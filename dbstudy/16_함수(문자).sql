...
--  3) INITCAP(칼럼) : INITIAL CAPITAL, 첫 글자만 대문자 나머지는 소문자로 변환
SELECT
        EMAIL
        ,UPPER(EMAIL)
        ,LOWER(EMAIL)
...
-- 4. 일부 반환
--  SUBSTR(칼럼, BEGIN, LENGTH) /SUBSTRING
--  칼럼 데이터의 BEGIN 위치보터 LENGTH개만큼 반환
