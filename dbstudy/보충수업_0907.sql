...
WHERE C.CUSTOMER_ID = O.CUSTOMER_ID -- 조인조건 먼저, 일반조건 나중에 배치
AND B.BOOK_ID = O.BOOK_ID -- 전체조회
AND O.ORDER_DATE NOT BETWEEN '14/07/04' AND '14/07/07';

-- 5. 모든 구매 고객의 이름과 총구매액(PRICE * AMOUNT)을 조회하시오. -- 이름으로 묶기, GROUP BY 같은 이름 다른사람이있을 경우를 대비해 이름으로 그룹화하는것은 바람직하지 않음 ID만으로 그룹화도 조치않음 ID와 NAME을같이 그룹화하기
-- 구매 이력이 있는 고객만 조회하시오.(주문내역에 있고 고객목록에)
-- 고객명  총구매액
-- 박지성  116000
-- 추신수  86000
-- 장미란  62000
...
-- 구매 이력이 없는 고객은 총구매액과 구매횟수를 0으로 조회하시오.(고객은 모두 조회, 주문내역은 있는 자료만 조회 = 왼쪽 외부조인)
...
SELECT C.CUSTOMER_NAME AS 고객명
    ...
    , COUNT(O.ORDER_ID) AS 구매횟수 --COUNT(*)(X)에는 고객정보가 포함되면 안됨(고객이름은 1건으로 셈) 주문할 때마다 부여되는 번호를 센다
...
-- 고객명  구매도서수
-- 김연아  2
SELECT C.CUSTOMER_NAME AS 고객명 -- 2개 조인
    , COUNT(*) AS 구매도서수
FROM CUSTOMER C INNER JOIN ORDERS O
ON C.CUSTOMER_ID = O.CUSTOMER_ID
WHERE C.CUSTOMER_NAME = '김연아'-- WHERE절 없애면 모든 고객
GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME;

-- 8. '박지성'이 구매한 도서를 발간한 출판사(PUBLISHER) 개수를 조회하시오. --3개 조인
-- 고객명  출판사수
-- 박지성  3
SELECT C.CUSTOMER_NAME AS 고객명
    ,COUNT(DISTINCT B.PUBLISHER) AS 출판사수 -- DISTINCT 칼럼 이름 앞에 들어감,COUNT안에 들어갈 수 있음
    FROM CUSTOMER C INNER JOIN ORDERS O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID INNER JOIN BOOK B
    ON B.BOOK_ID = O.BOOK_ID
...
FROM CUSTOMER C LEFT OUTER JOIN ORDERS O
ON C.CUSTOMER_ID = O.CUSTOMER_ID
GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME-- 중복제거
HAVING COUNT(O.ORDER_ID) = 0;-- 집계함수는 WHERE절안돼서 HAVING절
