...
public class BindingServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 2. 3개 영역을 사용
		//	1) ServletContext : 애플리케이션(프로젝트) 종료 전까지 내부 어디서든 접근해서 사용 가능
		//	2) HttpSession : 브라우저 종료 전까지 접근해서 사용 가능 (ex 로그인정보 저장)
		//	3) HttpServletRequest : 응답 전까지 접근해서 사용 가능
		// 3. 사용 메소드
		//	1) getAttribute('속성') : 해당 속성 값 가져오기 (오브젝트 타입으로 저장되어 캐스팅이 필요할 수 있음) 
