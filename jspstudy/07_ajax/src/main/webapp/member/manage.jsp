	...
	$.ajax({
		...
		success: function(resData){ // resData : {"count": 3, "members": [{}, {}, {}]}
			...
			$.each(resData.members, function(i, member){
				...
				//tr += '<td><input type="button" value="조회" class="btn_detail"><input type="hidden" value="'+ member.memberNo + '"></td>';
		...
		$('body').on('click', '.btn_detail', function(){
			$.ajax({
				...
				if(resData.exists){
					...
					//$(':radio [name=gender]).val=M].prof('checked', true)'); 안됨
			...
			$.ajax({
				...
				success:function(resData){ // resData : {"isSuccess": true}
					if(resData.isSuccess){
						...
						fn_getAllMembers(); // 목록을 새로 가져와서 갱신함 ajax불러옴(ajax가 ajax 불러오면 promise 대상이됨)
	...
	function fn_modify(){
		$('#btn_modify').click(function(){
			$.ajax({
				...
				// 응답
				dataType:'json',
				success: function(resData){// resData에 {"is:Success":true}가 들어있슴
					if(resData.isSuccess){
