...
public class BoardDao {
	...
	// singleton - pattern // 연습 많이하기
	// 오직 하나의 객체만을 생성할 수 있는 클래스
	// 한 번의 메모리만 할당하고 그 할당한 메모리에 대해 객체로 관리하기 위함
	// 외부에서 호출 시 new를 사용하지 않고 getInstance를 사용하여 객체를 생성하거나 반환받습니다.
	// 최초 호출이라면 new CompanyInfo()가 return 되어 사용할 것이고 두 번째 이후부터는 기존에 생성된 instance를
	// 반환받을 것입니다.
	...
	public void close(Connection con, PreparedStatement ps, ResultSet rs) {
		try {
			...
			if (con != null) {
				con.close();
			} // Connection Pool의 close()는 Connection 종료가 아닌 Connection 반환으로 동작
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public Board selectBoardByNo(int board_no) {
		...
		try {
			...
			rs = ps.executeQuery();// SELECT문은 executeQuery()사용
			if(rs.next()){ //상세보기, rs에 next()한번 필요
				board = new Board();
				board.setBoard_no(rs.getInt(1));// rs.getInt("Board_No")
				board.setTitle(rs.getString(2));// rs.getString("TITLE")
				board.setContent(rs.getString(3));// rs.getString("CONTENT")
	...
	// 6. 게시글 삭제
	public int deleteBoard(int board_no) {//int_board_no pk값
		int result = 0;
		try {
			con = dataSource.getConnection();
			sql = "DELETE FROM BOARD WHERE BOARD_NO = ?";
			ps = con.prepareStatement(sql);
			ps.setInt(1, board_no);// 첫번째물음표에 보드넘버 넣어라
			result=ps.executeUpdate();//executeDelete아님executeInsert아님
