...
public class MyController2 {
	...
	@GetMapping(value="board/detail1"
	          , produces=MediaType.APPLICATION_JSON_VALUE) //produces=MediaType 어디에나 하나만 있으면 생략가능
	public ResponseEntity<Board> detail1(HttpServletRequest request){
		return boardService.execute1(request);
	}
	@ResponseBody
	@GetMapping("board/detail2")//produces 가 없음을 주의 반환값 ResponseEntity에 관련 코드를 작성하였음
