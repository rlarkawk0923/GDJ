...
<script>
	...
	function fn_add(){
		$('#btn_add').click(function(){
			...
			let member = JSON.stringify({
				'id':$('#id').val(),
				'name': $('#name').val(),
				'gender': $(':radio[name=gender]:checked').val(),/* 선택된 라디오[젠더] */
				'address': $('#address').val()
			}); //dto와 매칭됨
			// 추가할 회원 정보를 DB로 보낸다.
			$.ajax({
				type: 'post',
				url: '${contextPath}/members',//let members
				data: member, // 파라미터 이름 없음(본문에 member를 포함시켜서 전송)
				contentType:'application/json', //member의 mimeType
				/* 응답 */
				dataType: 'json',
	...
	function fn_list(){
		$.ajax({
			type: 'get',
			url: '${contextPath}/members/page/' + page, // 주소= 9090/rest/members/page/1
			dataType: 'json',
			success: function(resData){
				$('#member_list').empty();
		...
		$('#btn_remove').click(function(){
			if(confirm('선택한 회원을 모두 삭제할까요?')){
				...
				for(let i = 0; i < $('.check_one').length; i++){
					if($($('.check_one')[i]).is(':checked')){// i 자바스크립트요소?
							memberNoList += $($('.check_one')[i]).val()+','; // 3,1
					}
				}
				memberNoList = memberNoList.substr(0, memberNoList.length -1); // 3,1 (substr 마지막 콤마 자르기)
				$.ajax({
					type: 'delete',
