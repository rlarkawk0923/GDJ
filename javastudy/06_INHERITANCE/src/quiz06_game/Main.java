...
public class Main {
	...
	public static void main(String[] args) {
		...
		while(unit1.isAlive() && unit2.isAlive()){// while(unit.getEnergy()> 0 && unit2.getEnergy 에너지가 0보다 크면 살아있음
			if(myTurn) {
				System.out.println(unit1.getName() + "의 공격!");
				unit1.attack(unit2); // unit1이 unit2를 공격한다.(탱크는 10% 확률로 상대를 한 번에 죽임)
				} else {
				System.out.println(unit1.getName() + "의 공격!");
				unit2.attack(unit1); // unit1이 unit2를 공격한다.(마린은 40% 확률로 상대를 한 번에 죽임)
				} 
			myTurn = !myTurn;
		...
		} else {System.out.println(unit2.getName() + "의 승리! 남은 에너지 : " + unit2.getEnergy());
